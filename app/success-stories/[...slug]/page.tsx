...
import { GET_SUCCESS_STORY_BY_PATH } from '@/lib/queries'
...
interface SuccessStoryByPathData {
  route: {
    entity: DrupalSuccessStory
  } | null
}

async function getSuccessStory(path: string): Promise<DrupalSuccessStory | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<SuccessStoryByPathData>({
      query: GET_SUCCESS_STORY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/success-stories/${slug.join('/')}`
  const item = await getSuccessStory(path)

  if (!item) {
    return { title: 'Success Story Not Found | Bright Futures' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).participantName && (
                    <div>
                      <dt className="text-sm text-gray-500">Participant Name</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).participantName}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
